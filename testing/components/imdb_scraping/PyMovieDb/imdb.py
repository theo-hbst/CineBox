...
class IMDB:
    ...
    def search(self, name, year=None, tv=False, person=False):
        ...
        for result in results:
            ...
            name = re.sub(r'[^\x00-\x7F]+', '', name)  # Handle special characters
    ...
    def get(self, url):
        ...
        # Handle special characters in movie names
        result['name'] = re.sub(r'[^\x00-\x7F]+', '', result.get('name', ''))
    ...
    def popular_movies(self, genre=None, start_id=1, sort_by=None, limit=25):
        """
         @description:- Helps to get popular movies starting from <start_id>.
         @parameter-1:- <str:genre> OPTIONAL, to filter results by genre.
         @parameter-2:- <int:start_id> DEFAULT=1, start id to show results (shows results from start_id to start_id+limit).
         ...
         @parameter-4:- <int:limit> DEFAULT=25, number of results to return.
        ...
        if genre is not None:
            ...
            url = f"https://www.imdb.com/search/title/?title_type=movie&genres={genre}&start={start_id}&count={limit}&sort={sort_by}"
        else:
            url = f"https://www.imdb.com/search/title/?title_type=movie&start={start_id}&count={limit}&sort={sort_by}"
